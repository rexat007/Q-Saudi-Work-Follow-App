...
// Filter out non-Arabic source
const cleanC = pool.eligibleC.filter((x: any) => arabicRegex.test(x.ar) && x.ar.trim().length > 1);
const cleanB = pool.eligibleB.filter((x: any) => arabicRegex.test(x.ar) && x.ar.trim().length > 1);
...
fs.writeFileSync('reports/block59-selected-candidates.json', JSON.stringify({
  finalC,
  finalB
}, null, 2), 'utf8');
