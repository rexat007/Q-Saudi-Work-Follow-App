...
const startMarker = 'Seeds all required master datasets into IndexedDB with explicit versions and ISO timestamps.\n   */';
const endMarker = '  // ---------------- Query Cached Data ---------------- //';
...
if (startIndex !== -1 && endIndex !== -1) {
  ...
  content = content.substring(0, startIndex + startMarker.length) + replacement + content.substring(endIndex);
  fs.writeFileSync('src/services/offline/offlineCache.service.ts', content);
  console.log('Successfully replaced seed method range in offlineCache.service.ts');
} else {
