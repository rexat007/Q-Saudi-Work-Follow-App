#!/bin/bash
# Replaces seedAllMasterData logic in offlineCache.service.ts

sed -i '/import { DEFAULT_PROJECTS } from/d' src/services/offline/offlineCache.service.ts
sed -i '/import { SAMPLE_QUALITY_CONTEXT } from/d' src/services/offline/offlineCache.service.ts
sed -i '/import { MASTER_PRICING_RULES, MasterPricingRule } from/d' src/services/offline/offlineCache.service.ts

# Add pricingService and adminConsoleService imports if they aren't there
grep -q "import { adminConsoleService }" src/services/offline/offlineCache.service.ts || sed -i '1s/^/import { adminConsoleService } from "..\/adminConsole.service";\n/' src/services/offline/offlineCache.service.ts
grep -q "import { pricingService }" src/services/offline/offlineCache.service.ts || sed -i '1s/^/import { pricingService } from "..\/pricing.service";\n/' src/services/offline/offlineCache.service.ts

# Replace seedAllMasterData method
...
sed -i '/public async seedAllMasterData(): Promise<void> {/,/^  }/c\
$(cat replacement.ts | sed -e '\''s/[\/&]/\\&/g'\'')' src/services/offline/offlineCache.service.ts
# We can use node to replace it more robustly
