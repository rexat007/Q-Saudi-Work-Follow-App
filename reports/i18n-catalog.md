...
**Generated At:** 2026-09-14T05:21:08.497Z
...
| :--- | :--- | :--- |
| **Total Files Scanned** | 270 | TypeScript and TSX files analyzed in `src/` |
| **Total Extracted Candidates** | 34837 | Total strings and text nodes parsed |
| **Real User-Facing Texts** | 9088 | Confirmed visible Arabic and UI texts |
| **Likely User-Facing Texts** | 358 | English UI text in visible elements |
| **Ambiguous Texts** | 8201 | Short or context-isolated tokens for manual review |
| **Technical Strings** | 7441 | CSS classes, paths, enums, regexes, operators |
| **Non-User-Facing Strings** | 9749 | Internal IDs, collection names, database keys |
| **Distinct Proposed Keys** | 8052 | Hierarchical keys generated (e.g. `shared.actions.save`) |
| **Duplicate Groups** | 4135 | Sets of identical/normalized recurring texts |
| **Semantic Conflict Groups** | 572 | Identical texts requiring distinct contextual keys |
| **Keys Requiring Review** | 10130 | Entries flagged with special cases or ambiguity |
| **High / Critical Risk Hotspots** | 2272 | Concatenations, templates, or business data overlaps |

## 2. Category Distribution

| Semantic Category | User-Facing Strings | Description |
| :--- | :--- | :--- |
| `other` | 4255 | Classified operational domain |
...
| `navigation` | 138 | Classified operational domain |
...
- **Total Directional Usages Detected:** 352
- **Must Migrate (Physical to Logical):** 248 (e.g. `text-left` -> `text-start`, `pl-` -> `ps-`)
- **Probably Safe / Technical:** 104

### Directional Classes Sample (First 15 Entries)

| File | Line | Class / Icon | Type | Risk | Suggested Replacement |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `src/App.tsx` | 744 | `ChevronLeft` | icon | `MUST_MIGRATE` | `Logical directional flip required (e.g. rtl:rotate-180)` |
| `src/App.tsx` | 767 | `ChevronLeft` | icon | `MUST_MIGRATE` | `Logical directional flip required (e.g. rtl:rotate-180)` |
...
- **Template Literals (`${...}`):** 647
- **Interpolation Placeholders (`{...}`):** 254
- **Mixed Numbers & Units:** 891
- **Currency String Usages:** 410
- **Unit String Usages:** 845
- **Protected Business Field Matches:** 343
